...
using namespace std;
using namespace uriBase;            //  base raster image classes
using namespace uriVideoSources;    //  image input
using namespace uriMovieEditing;    //  image output

#error --> Replace this source file by ../../<myMainSourceFile>.cpp
...
//
#if	URIVL_OS_MACOS
	#define	kNameFile2	"../../../../../../../../Data_Samples/Color/umbrellas_480x400.tga"
	#define	kNameFile3	"../../../../../../../../Data_Samples/Color/a-port-en-bessinB_300x400.ppm"
	#define	kNameFile4	"../../../../../../../../Data_Samples/Gray/Lena_256x256.pgm"
#elif URIVL_OS_WIN32
	#define	kNameFile2	"../../../../Data_Samples/Color/umbrellas_480x400.tga"
	#define	kNameFile3	"../../../../Data_Samples/Color/a-port-en-bessinB_300x400.ppm"
	#define	kNameFile4	"../../../../Data_Samples/Gray/Lena_256x256.pgm"
...
void loadImages(void)
{
    ...
    if (gMyImage[1] != NULL)
    {
        ...
        unsigned char	**rgba = gMyImage[1]->getRaster2D();
        ...
        for (int i=0; i<nbRows; i++)
            ...
            {
                rgba[i][4*j] = static_cast<unsigned char>(floor((i-n/2.)*(i-n/2.)/120. + 20));
                rgba[i][4*j+1] = static_cast<unsigned char>(j/4 + 30);
                rgba[i][4*j+2] = static_cast<unsigned char>(floor(127*(cos(0.06*i)*sin(0.1*j)+0.5)));
    ...
    if (gMyImage[2] !=NULL)
    {
        unsigned char **gray = gMyImage[2]->getRaster2D();
    ...
    myReader = new ImageReader(kNameFile2);

    gMyImage[3] = myReader->getFrame(false);
    delete myReader;
    ...
    //-------------------------------------------------------------
    //	Image 4 is read from a PPM file
    //-------------------------------------------------------------
    //  I can also create the reader w/o passing any file info
    myReader = new ImageReader( );
    //  and later on specify a file path, FSSPec, or select the file to read
    //  through a GUI dialog
    myReader->setSource(kNameFile3);
    ...
    //-------------------------------------------------------------
    //	Image 5 is read from a PGM file (gray-level image)
    //-------------------------------------------------------------
    //  I can also reuse the same ImageReader and simply set a new source
    myReader->setSource(kNameFile4);
    gMyImage[5] = myReader->getFrame(true);
    if (gMyImage[5] ==NULL)
    {
    ...
    //  and I can reuse the same ImageWriter with a new destination
    myWriter->setDestination("./Output/im1.ppm");
    myWriter->outputFrame(gMyImage[1]);
...
void myReshape(int w, int h)
{
    ...
    DrawableObject2D::setDefaultWindowDimensions(gMyImage[gCurrentImg]->getWidth(),
...
int main(int argc, char** argv)
{
    ...
    glutReshapeWindow(gMyImage[0]->getWidth(),
                      gMyImage[0]->getHeight());

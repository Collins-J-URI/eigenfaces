/*  NAME:
    ...
    DESCRIPTION:
        implementation of the uriVisionLib Cone3D class
...
//	Redefinition of the class's static elements
//
bool	Cone3D::initCtSt_ = false;
GLfloat Cone3D::ct_[kVeryHighDefCircPts],
		Cone3D::st_[kVeryHighDefCircPts];



//==================================================================================
//	Cone3D constructors and destructor
//==================================================================================

/*----------------------------------------------------------------------------------+
|	Constructor:	Cone3D(GLfloat* origin, GLfloat** axes, bool isClosed)			|
|																					|
...
|					isClosed	should the bottom face be drawn?					|
|																					|
...
|					is at origin + axes[2]											|
+----------------------------------------------------------------------------------*/
Cone3D::Cone3D(GLfloat* origin, GLfloat** axes, bool isClosed)
		:	GraphicComponent3D(),
			//
			...
			closedTop_(false)
{
	...
	try {
		...
		deleteArrays_();
		e.appendToMessage("called from Cone3D constructor.");
		throw e;
	}
}

/*----------------------------------------------------------------------------------+
|	Constructor:	Cone3D(GLfloat* origin, GLfloat** botAxes, GLfloat* dirAxis,	|
|							GLfloat** topAxes, bool isClosed)						|
|																					|
|	Parameters:		origin		coordinates of the origin (bottom' center)			|
...
|					init() method to initialize these arrays.						|
+----------------------------------------------------------------------------------*/
Cone3D::Cone3D(GLfloat* origin, GLfloat** botAxes, GLfloat* dirAxis,
					GLfloat** topAxes, bool isClosed)
		:	GraphicComponent3D(),
			//
			vertex_(new GLfloat** [kVeryHighDefSlabs+1]),
...
/*----------------------------------------------------------------------------------+
|	Constructor:	Cone3D(GLfloat* origin, GLfloat* dirAxis, GLfloat bottomRad,	|
|								GLfloat topRad, bool isClosed)						|
|																					|
|	Parameters:		origin		coordinates of the origin (bottom' center)			|
|					dirAxis		coordinates of the cylinder's axis.					|
...
|					init() method to initialize these arrays.						|
+----------------------------------------------------------------------------------*/
Cone3D::Cone3D(GLfloat* origin, GLfloat* dirAxis, GLfloat bottomRad, GLfloat topRad,
						bool isClosed)
		:	GraphicComponent3D(),
			//
			vertex_(new GLfloat** [kVeryHighDefSlabs+1]),
			...
			closedTop_(isClosed)
{
	...
	catch (...)
	{
		...
		FAIL_CONDITION( true,
						...
						"Invalid array passed to Cone3D constructor");
	}
...
Cone3D::Cone3D(const Cone3D& theBox)
		:	GraphicComponent3D(),
			//
			vertex_(NULL),
			normal_(NULL),
			...
			closedTop_(false)
{
    FAIL_CONDITION( true,
                    ...
                    "Cone3D copy constructor not implemented.");
}



/*----------------------------------------------------------------------------------+
|	Destructor:		~Cone(void)														|
|																					|
|	Parameters:		None															|
|																					|
...
const Cone3D& Cone3D::operator = (const Cone3D& obj)
{
    FAIL_CONDITION( true,
                    kFunctionNotImplemented,
                    "Cone3D copy operator not implemented.");
...
|	Returns:		Nothing															|
|																					|
|	Description:	Adjusts private variables for proper display of the cone		|
|																					|
+----------------------------------------------------------------------------------*/
void Cone3D::setResolution(ResolutionLevel res)
{
...
|																					|
+----------------------------------------------------------------------------------*/
void Cone3D::setResolution(ResolutionLevel circRes, ResolutionLevel slabRes)
{
	switch (circRes)
	{
...
|																					|
+----------------------------------------------------------------------------------*/
void Cone3D::init_(GLfloat* origin, GLfloat** a)
{
...
|																					|
+----------------------------------------------------------------------------------*/
void Cone3D::init_(GLfloat* origin, GLfloat** botAxes, GLfloat* dirAxis,
					GLfloat** topAxes)
{
	GLfloat	dirStep = 1.f/kVeryHighDefSlabs,
			//	X, Y, and Z coordinates of the radial and normal directions.
...
void Cone3D::deleteArrays_(void)
{
	if (vertex_!=NULL)
	{
		for (int i=0; i<=kVeryHighDefSlabs; i++)

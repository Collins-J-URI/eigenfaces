/*  NAME:
        LinearSolver_Jacobi.cpp
 
    DESCRIPTION:
        implementation of the uriVisionLib LinearSolver_Jacobi class
...
//------------------------------
const double LinearSolver_Jacobi::STOP_ITERATION_THRESHOLD = 1.E-5;
...
LinearSolver_Jacobi::LinearSolver_Jacobi(int nbRows, int nbCols)
			...
			invDiag_(NULL)
{
    if (nbRows != nbCols)
    {
        ...
        FAIL_CONDITION( true,
                        ...
                        "Nonsquare SLE assigned to a LinearSolver_Jacobi");
    }
	setMatrixIsLocal_(true);
}
catch (ErrorReport& e) {
	e.appendToMessage("called by LinearSolver_Jacobi constructor");
	throw e;
}


LinearSolver_Jacobi::LinearSolver_Jacobi(Matrix* A, bool preserveMatrix)
			...
			invDiag_(NULL)
{
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
                    "Null matrix assigned to a LinearSolver_Jacobi");
	...
	if (nbRows != nbCols)
	{
		FAIL_CONDITION( true,
						kInvalidParameterError,
						"Nonsquare matrix assigned to a LinearSolver_Jacobi");
	}
...
LinearSolver_Jacobi::LinearSolver_Jacobi(const LinearSolver_Jacobi& obj)
		:   LinearSolver(),
			//
			A_(NULL),
			currentX_(NULL),
			...
			invDiag_(NULL)
{
	FAIL_CONDITION (true,
					...
					"LinearSolver_Jacobi copy constructor not inplemented");
}



LinearSolver_Jacobi::~LinearSolver_Jacobi(void)
{
...
const LinearSolver_Jacobi& LinearSolver_Jacobi::operator = (const LinearSolver_Jacobi& obj)
{
	FAIL_CONDITION (true,
					kFunctionNotImplemented,
					"LinearSolver_Jacobi copy opertator not inplemented");
	return *this;
}



void LinearSolver_Jacobi::setMatrix(Matrix* A, bool preserveMatrix)
{
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
                    "Null matrix assigned to a LinearSolver_Jacobi");
...
LinearSolverRecord* LinearSolver_Jacobi::solve(Matrix *B)
{
    return solve(B, 0);
}

LinearSolverRecord* LinearSolver_Jacobi::solve(Matrix *B, int incrImprov)
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term sent to a LinearSolver_Jacobi");
...
void LinearSolver_Jacobi::solve(Matrix *B, LinearSolverRecord *record)
{
    solve(B, record, 0);
}

void LinearSolver_Jacobi::solve(Matrix *B, LinearSolverRecord *record, int incrImprov)
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term sent to a LinearSolver_Jacobi");
    FAIL_CONDITION( record == NULL,
                    kNullParameterError,
                    "Null LinearSolverRecord term sent to a LinearSolver_Jacobi");
    const int nbRows = B->getNbRows(),
              nbCols = B->getNbCols();
    FAIL_CONDITION( nbRows != getNbRows_(),
                    kInvalidParameterError,
...
LinearSolverRecord* LinearSolver_Jacobi::solve(Matrix* A, Matrix* B, bool preserveMatrix) 
{
    return solve(A, B, 0, preserveMatrix);
}


LinearSolverRecord* LinearSolver_Jacobi::solve(Matrix* A, Matrix* B, int incrImprov,
                                           bool preserveMatrix) 
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term assigned to a LinearSolver_Jacobi");
    const int nbRows = B->getNbRows(),
              nbCols = B->getNbCols();
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
    ...
    FAIL_CONDITION( aRows != nbRows,
                    kInvalidParameterError,
                    "Matrix and right-side term dimensions dont match for LinearSolver_Jacobi");
...
void LinearSolver_Jacobi::solve(Matrix* A, Matrix* B, LinearSolverRecord *record,
                            bool preserveMatrix) 
{
                            ...
                            int incrImprov, bool preserveMatrix) 
{
    ...
    if (record->sol != NULL)
    {
        FAIL_CONDITION( (nbRows != record->sol->getNbRows()) || 
                        ...
                        kInvalidParameterError,
                        "right-side term and solution's dimensions dont match for LinearSolver_Jacobi");
    }
...
void LinearSolver_Jacobi::solve_(Matrix* A, Matrix* B, LinearSolverRecord *record,
...
LinearSolverRecordStatus LinearSolver_Jacobi::checkMatrixStatus_(Matrix* A, double* invDiag)
{
	const double* const* a = A->getMatrix(); 
    const int nbRows = A->getNbRows();

...
*/
#ifndef URIVL_LINEAR_SOLVER_LU_H
#define URIVL_LINEAR_SOLVER_LU_H
...
namespace uriVL
{
    /** Class that implement Crout's version of the LU factorization algorithm for solving
     *  SLEs.  This implementation was certainly influenced by that found in the Numerical
     *  Recipes in C, but it is a complete recoding, not just an adaptation.
     *
     ...
     *                      University of Rhode Island
     */
    class URIVL_EXPORT LinearSolver_LU : public LinearSolver
    {
        public:

            /** Space is allocated for a new LinearSolver_LU object. That matrix must have
             ...
             */
            LinearSolver_LU(int nbRows, int nbCols);


            /** Constructor.  A LinearSolver_LU is created for a specific matrix.
             ...
             */
            LinearSolver_LU(Matrix* A, bool preserveMatrix=false);
            
//            /** Constructor.  A LinearSolver_LU is created for a specific dimension of the SLE
//             *  and number of clumns of the right-side term.  This is the version of the 
...
//             *                          advance.
//             */
//            LinearSolver_LU(int n, int nbRighSideCols=1);
            
            
            /** Destructor
             */
            ~LinearSolver_LU(void);
        ...
        private:
             ...
             *  ill-conditioned
             */
            static const double ILL_COND_PIVOT_THRESHOLD;
        
            /** Matrix that stores the LU factorization of the matrix
             *  for which this object was created
             */
            Matrix* LU_;
            
            /** Permutation array for this factorization object
             */
            int*    p_;

            /** Array that stores row information for pivoting
             */
            double*    v_;
             ...
             */
            LinearSolver_LU(const LinearSolver_LU& obj);

            /** Copy operator: disabled.
			 *	@param	obj		reference to the object to copy
             */
            const LinearSolver_LU& operator = (const LinearSolver_LU& obj);
            ...
            /** Computes the LU factorization of an SLE
             *  @param  n   the dimension of the SLE
             *  @param  a   the matrix of the SLE
             *  @param  p   the preallocated index permutation array
             *  @param  v   a preallocated array for pivoting calculations
             *  @return     a matrix singularity status
             */
            static LinearSolverRecordStatus factorize_(int n, double* const*const a, 
                                               int* p, double* v_);

            /** Solves a unit-diagonal lower-triangular SLE
             *  @param  m   the dimension of the SLE's matrix
             *  @param  n   the number of columns of the right-side term
             *  @param  l   the lower-triangular matrix of the SLE
             *  @param  b   the SLE's right-side term
             *  @param  p   the preallocated index permutation array
             *  @param  y   the solution matrix/vector
             */
            static void forwardSubstitution_(int m, int n, double* const*const l, double* const*const b,
                                      int* p, double* const*const y);

            /** Solves an upper-triangular SLE
             *  @param  m   the dimension of the SLE's matrix
             *  @param  n   the number of columns of the right-side term
             *  @param  u   the lower-triangular matrix of the SLE
             *  @param  y   the SLE's right-side term
             *  @param  p   the preallocated index permutation array
             *  @param  x   the solution matrix/vector
             */
            static void backSubstitution_(int m, int n, double* const*const u, double* const*const y,
                                   int* p, double* const*const x);

    };
}

#endif  //  LINEAR_SOLVER_LU_H

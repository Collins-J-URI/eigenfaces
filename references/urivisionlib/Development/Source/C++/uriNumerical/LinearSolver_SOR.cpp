/*  NAME:
        LinearSolver_SOR.cpp
 
    DESCRIPTION:
        implementation of the uriVisionLib LinearSolver_SOR class
...
//------------------------------
const double LinearSolver_SOR::STOP_ITERATION_THRESHOLD = 1.E-5;
...
LinearSolver_SOR::LinearSolver_SOR(int nbRows, int nbCols)
			...
			invDiag_(NULL)
{
    if (nbRows != nbCols)
    {
        ...
        FAIL_CONDITION( true,
                        ...
                        "Nonsquare SLE assigned to a LinearSolver_SOR");
    }
	setMatrixIsLocal_(true);
}
catch (ErrorReport& e) {
	e.appendToMessage("called by LinearSolver_SOR constructor");
	throw e;
}


LinearSolver_SOR::LinearSolver_SOR(Matrix* A, bool preserveMatrix)
			...
			invDiag_(NULL)
{
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
                    "Null matrix assigned to a LinearSolver_SOR");
	...
	if (nbRows != nbCols)
	{
		FAIL_CONDITION( true,
						kInvalidParameterError,
						"Nonsquare matrix assigned to a LinearSolver_SOR");
	}
...
LinearSolver_SOR::LinearSolver_SOR(const LinearSolver_SOR& obj)
		:   LinearSolver(),
			//
			A_(NULL),
			currentX_(NULL),
			...
			invDiag_(NULL)
{
	FAIL_CONDITION (true,
					...
					"LinearSolver_SOR copy constructor not inplemented");
}



LinearSolver_SOR::~LinearSolver_SOR(void)
{
...
const LinearSolver_SOR& LinearSolver_SOR::operator = (const LinearSolver_SOR& obj)
{
	FAIL_CONDITION (true,
					kFunctionNotImplemented,
					"LinearSolver_SOR copy opertator not inplemented");
	return *this;
}



void LinearSolver_SOR::setMatrix(Matrix* A, bool preserveMatrix)
{
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
                    "Null matrix assigned to a LinearSolver_SOR");
...
LinearSolverRecord* LinearSolver_SOR::solve(Matrix *B)
{
    return solve(B, 0);
}

LinearSolverRecord* LinearSolver_SOR::solve(Matrix *B, int incrImprov)
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term sent to a LinearSolver_SOR");
...
void LinearSolver_SOR::solve(Matrix *B, LinearSolverRecord *record)
{
    solve(B, record, 0);
}

void LinearSolver_SOR::solve(Matrix *B, LinearSolverRecord *record, int incrImprov)
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term sent to a LinearSolver_SOR");
    FAIL_CONDITION( record == NULL,
                    kNullParameterError,
                    "Null LinearSolverRecord term sent to a LinearSolver_SOR");
    const int nbRows = B->getNbRows(),
              nbCols = B->getNbCols();
    FAIL_CONDITION( nbRows != getNbRows_(),
                    kInvalidParameterError,
...
LinearSolverRecord* LinearSolver_SOR::solve(Matrix* A, Matrix* B, bool preserveMatrix) 
{
    return solve(A, B, 0, preserveMatrix);
}


LinearSolverRecord* LinearSolver_SOR::solve(Matrix* A, Matrix* B, int incrImprov,
                                           bool preserveMatrix) 
{
    FAIL_CONDITION( B == NULL,
                    kNullParameterError,
                    "Null right-side term assigned to a LinearSolver_SOR");
    const int nbRows = B->getNbRows(),
              nbCols = B->getNbCols();
    FAIL_CONDITION( A == NULL,
                    kNullParameterError,
    ...
    FAIL_CONDITION( aRows != nbRows,
                    kInvalidParameterError,
                    "Matrix and right-side term dimensions dont match for LinearSolver_SOR");
...
void LinearSolver_SOR::solve(Matrix* A, Matrix* B, LinearSolverRecord *record,
                            bool preserveMatrix) 
{
                            ...
                            int incrImprov, bool preserveMatrix) 
{
    ...
    if (record->sol != NULL)
    {
        FAIL_CONDITION( (nbRows != record->sol->getNbRows()) || 
                        ...
                        kInvalidParameterError,
                        "right-side term and solution's dimensions dont match for LinearSolver_SOR");
    }
...
void LinearSolver_SOR::solve_(Matrix* A, Matrix* B, LinearSolverRecord *record,
...
LinearSolverRecordStatus LinearSolver_SOR::checkMatrixStatus_(Matrix* A, double* invDiag)
{
	const double* const* a = A->getMatrix(); 
    const int nbRows = A->getNbRows();
